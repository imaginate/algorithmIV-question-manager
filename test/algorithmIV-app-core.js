...
/**
 * @license
 * Algorithm IV (v1.1.0) (learn@algorithmiv.com)
 * Section: Core Module for App
 ...
 * -----------------------------------------------------------------------------
 * the core module for implementing the question management app
 **
 * annotation:
 * - Closure Compiler specific JSDoc
 * - see developers.google.com/closure/compiler/
 **
 * structure:
 * - an outline of all the module's variables, methods, and classes
...
(function(/** Window */ window, /** core */ core) {
  "use strict";


/* -----------------------------------------------------------------------------
 * | NEW SECTION: Public API                                                |
 * v ---------------------------------------------------------------------- v */
 ...
 * v ---------------------------------------------------------------------- v */
  ...
  /**
   * -----------------------------------------------------
   * Public Variable (core)
   * -----------------------------------------------------
   * holds the public methods for the core module
   * @typedef {{
   ...
   * @param {?Object} questions - The user's settings
   */
  core.init = function(config, sources, categories, questions) {
    if (!_initialized) {
      _initialized = true;
 ...
 * v ---------------------------------------------------------------------- v */
  ...
  var LoadApp = (function() {
    ...
    var _return = {
       ...
       * param: questions
       */
      init: function(config, categories, sources, questions) {
        // OPEN: LoadApp Group
        DEBUG.LoadApp.group && console.groupCollapsed(
          'GROUP: LoadApp ' +
    ...
    function init(newConfig, categories, sources, questions) {
      ...
      DEBUG.LoadApp.fail && console.assert(
      ...
      if ( SetConfiguration.init(newConfig, categories, sources, questions) ) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
          ...
          appendError();
          // CLOSE: LoadApp Group
          DEBUG.LoadApp.group && console.groupEnd();
        });
      }
    }
    ...
    function loadModule(worker) {
      // Debuggers
      DEBUG.LoadApp.call && console.log(
        'CALL: LoadApp.loadModule(%s)', worker
      );
      DEBUG.LoadApp.fail && console.assert(
        typeof worker === 'boolean',
        'FAIL: LoadApp.loadModule() ' +
        'Note: Incorrect argument operand.'
      );
      // Declare method variables
