    ...
    },
    {
      ...
      complete: false,
       ...
       problem: 'Given an array of random numbers, create a binary search tree with the median as the root. Then convert the binary search tree into a doubly-linked list that is sorted in ascending or descending order and return the first node in the list. Do the sort and conversion in place<span style="margin:0 12px">&ndash;</span>i.e. the memory complexity of your algorithms should be <em style="margin:0 2px">&Omicron;</em>(1).' +
                ...
                '<style>' +
                  ...
                  '.aIV-exQ3-table td {padding:0;margin:0;text-align:center;verticle-align:middle}' +
                '</style>' +
                '<table class="aIV-exQ3-table">' +
                  ...
                  '<tr>'  +
                    ...
                    '<td>' +
                      '<table class="aIV-exQ3-table" style="margin:0 auto">' +
                        '<tr>'  +
                          '<td></td>'  +
                          '<td></td>'  +
                          '<td>7</td>' +
      ...
      solution: function() {
        ...
        function setResults() {
          ...
          // The number of dashes at top of BST
          // The loop index
          // The current row
          // The string for the BST
          var rows, columns, dashes, i, row, strings, side;
          ...
          for (i=0; i<rows; i++) {
            ...
            // Set side cells
            side = ( (i === 0) ?
              Math.floor(columns / 2) : (i === 1) ?
                side - dashes - 1 : side - 2
            );
            // If (root)
//            treeString = ( (side === 0 && !!row) ?
//              '<td colspan="' +  + '>' : '<td>'
//            );
            // Close row
            treeString += '</tr>';
        ...
        // Adds each structure to the results
        setResults.addVals = function() {
          results.vals = vals.join(',');
        };
        setResults.addTree = function() {
        ...
        setResults.addList = function() {
          results.list = list.join('&nbsp;&nbsp;&lrarr;&nbsp;&nbsp;');
        };
        ...
        setResults.addVals();
        sortValues();console.log('vals= [ %s ]', vals.join(','));
        createTree();console.log('tree= %O', tree);
        setResults.addTree();
        createList();console.log('list= %O', list);
        setResults.addList();
        return;
      }
    },
